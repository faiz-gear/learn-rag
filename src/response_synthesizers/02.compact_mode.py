...
compact_synthesizer = get_response_synthesizer(response_mode=ResponseMode.COMPACT)
...
# 使用 compact 响应生成器生成响应
# compact模式： 系统默认的合成模式，会将nodes进行合并，然后使用refine模式生成response
response = compact_synthesizer.synthesize(
